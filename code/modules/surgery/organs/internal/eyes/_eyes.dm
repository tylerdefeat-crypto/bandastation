...
/obj/item/organ/eyes/proc/generate_body_overlay(mob/living/carbon/human/parent, obj/item/bodypart/limb)
	...
	if (scarring & RIGHT_EYE_SCAR)
		var/mutable_appearance/right_scar = mutable_appearance(eye_icon, "eye_scar_right", -EYES_LAYER, parent || limb) // BANDASTATION EDIT - Feat: Augmentation
		right_scar.color = limb.draw_color
		overlays += right_scar

	if (scarring & LEFT_EYE_SCAR)
		var/mutable_appearance/left_scar = mutable_appearance(eye_icon, "eye_scar_left", -EYES_LAYER, parent || limb) // BANDASTATION EDIT - Feat: Augmentation
