...
| D | ~~`rust_labyrinth/code/content/*`~~ | **❌ УДАЛЕНО.** Процедурный спавн контента (`labyrinth_content_module` + piston_trap/forge_decor/horror_ambience) убран целиком. Весь контент маппер кладёт в DMM руками. См. «Наполнение секторов» ниже. | ❌ Removed |
| E | `rust_labyrinth/maps/*.dmm` | DMM-куски секторов: geometry + намапленные механики (плиты/ловушки/двери/терминалы). Пул по типу, случайный выбор на старте | ✅ Авторские карты делает маппер |
...
Map modules (`.dmm`) go in `rust_labyrinth/maps/`. The mapper authors each sector DMM with **both** geometry (walls, floors, airlocks) **and** the gameplay mechanics (pressure plates, hazards, doors, terminals) placed and wired directly in the map. There is **no** procedural content generation — code only loads the DMM, sets up the sector's controllers/area, and registers whatever mechanics the map already contains.
...
- DMM = geometry **plus** mapped mechanics (plates, hazards, doors, terminals), authored by the mapper.
- After DMM loads, `on_loaded()` calls `set_turfs_to_area(sector.get_turfs(), zone_area)` to assign all sector tiles to a custom `/area/awaymission/rust_labyrinth/*` instance, then `_register_mapped_hazards()` adopts every map-placed hazard into the sector's `sync_controller`.
- Custom areas control ambient sound, lighting level, atmosphere.
- **No procedural content.** Content modules were removed — nothing is spawned at runtime. The mapper places and wires everything in the DMM.
...
### Наполнение секторов (рандомизация пулов, заменила Content Modules)

> ⚠️ **Content Modules удалены.** Каталог `code/content/`, тип `/datum/labyrinth_content_module`, подтипы `piston_trap`/`forge_decor`/`horror_ambience`, процедуры `apply_content_module`/`clear_content_module`/`_default_content_module` и поле `applied_module` — **всё убрано**. Никакого процедурного спавна. Контент маппер кладёт в DMM руками; код только грузит карту и регистрирует то, что в ней уже есть.

**Рандомизация на старте** (`code/setup/default_layout.dm`):
- `GLOB.labyrinth_sector_pools` — assoc-лист `"[sector_type]"` → `list(mappaths)`. Маппер добавляет свои DMM в пул нужного типа.
- `labyrinth_pick_sector_map(sector_type)` → `pick()` из пула, или `null` если пул пуст.
- `register_default_labyrinth_layout()` идёт по фиксированному `LABYRINTH_DEFAULT_LAYOUT` (типы/кольца зафиксированы) и на КАЖДЫЙ слот берёт **случайную** карту из пула его типа. Поле `mappath` в layout-списке игнорируется.
- Пустой/отсутствующий пул → `mappath = null` → `generate_sector_geometry()` (процедурная geometry-коробка) как fallback.
- Итог: расположение типов стабильно, но геометрия/механика конкретной комнаты меняется от раунда к раунду в пределах типа.

**Регистрация намапленных ловушек:** механики в DMM проходят `Initialize()` внутри `tmpl.load()` — до создания `sync_controller` в `on_loaded()`. В `wiring` они регистрируются сразу (он уже есть на датуме), а в `sync_controller` их добирает `rust_sector._register_mapped_hazards()` (свип `get_turfs()` после создания контроллера). Поэтому консоль-инспектор и `pulse/loop_all` видят карту-ловушки.
...
- **Read-only инспектор + управление.** Показывает 5×5 сетку (цвет по типу/статусу), правую панель выбранного сектора (rust-слайдер, Pulse/Loop All/Stop All, teleport, force-corrosion) и список намапленных ловушек/дверей зоны с пер-объектными кнопками Fire/Loop/Stop и Open/Close. **Не генерирует контент** — никаких apply/clear модулей и attach/clear пазлов (убрано).
...
Лабиринт грузится в `SSmapping/Initialize()` через extension-хук `code/mapping_hook.dm` — тот же этап, что lavaland/ruins, **только при старте сервера, никогда не в середине раунда** (процедурная генерация в раунде роняет сервер). Хук: `. = ..()` → `initialize_labyrinth()` → `register_default_labyrinth_layout()` → цикл `load_sector()` с `CHECK_TICK`. `register_default_labyrinth_layout()` на каждый слот тянет **случайный** DMM из `GLOB.labyrinth_sector_pools` по типу (см. «Наполнение секторов»). Admin verb «Initialize» оставлен для ручного перезапуска, но в норме не нужен.

> ⚠️ Никакой ин-гейм перестройки карты по кнопке. Раньше существовал `reload_sector` + verb «Load Puzzle Demo» — **удалены**. Менять наполнение можно только пересборкой + рестартом; в раунде сектора не пересобираются.

DMM-секторы 46×46 лежат в `maps/`: `test_corridor/trap/forge/boss/transition.dmm` + пример с механикой `test_puzzle.dmm` (в пуле TRAP). Area прописаны прямо в DMM (`/area/awaymission/rust_labyrinth/*`), плюс `on_loaded()` всё равно переназначает их через `set_turfs_to_area()`.
...
> Сборка проверяется **только** через `BUILD.cmd` (определяет `CBT`). Голый `dm.exe` неподдерживаем — даёт рантайм-крэш smoothing (`json_decode` на ~77% загрузки). См. раздел Build & Run.
...
## ⚡ Оптимизация производительности — план на следующую сессию

> Контекст: сейчас лабиринт **сильно лагает**. Аудит провёл, корневая причина найдена. Ниже — диагноз + конкретный рефактор по образцу ванильных модулей. **Не реализовано**, делаем в следующей сессии. Билд проверять только `BUILD.cmd` (CBT).

### Корневая причина (важнее всего)

**Всё крутится с round-start и НИКОГДА не останавливается, даже когда в лабиринте нет ни одного игрока.** Лабиринт грузится в `SSmapping/Initialize()` и висит весь раунд. На него навешано N бесконечных self-loop'ов (`set waitfor = FALSE` + `while(...) SLEEP_NOT_DEL`), каждый из которых — отдельная спящая прока, просыпающаяся по таймеру и делающая работу вне зависимости от присутствия игроков:

| Источник | Файл | Что делает каждый тик | Масштаб |
|----------|------|----------------------|---------|
| **Contact-loop пил** | `mechanics/hazard.dm` `start_contact_loop()` | `get_turf` + `get_step` по `live_offsets`, скан contents тайла | 1 прока **на каждую пилу**, тик 1 с, вечно |
| **Corrosion sweep** | `mechanics/corrosion.dm` `_loop()`→`_sweep()` | **`block()` строит список 46×46 = 2116 турфов** и сканит contents КАЖДОГО | 1 прока **на сектор** (до 25), тик 10 с, вечно |
| **Hazard auto-loop** | `hazard.dm` `start_loop()` | полный warn→strike→reset цикл | 1 прока на зацикленную ловушку |

25 секторов × (corrosion + куча hazard-loop'ов + contact-loop на каждую пилу) = десятки-сотни спящих прок + пересборка 2116-элементных списков, **постоянно, на пустой карте**. Это и есть лаг.

### Эталон в билде: `code/game/turfs/open/lava.dm`

Лава — это ровно наш кейс (среда, которая жжёт стоящих на ней мобов). Делает так:
- `Entered(arrived)` → если есть кого жечь, `START_PROCESSING(SSobj, src)`. **Обработка стартует только когда на тайл зашли.**
- `process(seconds_per_tick)` → жжёт; если жечь больше нечего → `return PROCESS_KILL` → **подсистема сама выкидывает турф из обработки.** Пустая лава = 0 нагрузки.
- `Exited()` — очистка.
- `cache_burn_check()` — кэширует проверку per-atom через `WEAKREF`, не пересчитывает каждый тик.

Итог: нагрузка пропорциональна числу **занятых** тайлов, а не общему числу тайлов. Спящих прок — ноль.

### Рефактор (по приоритету)

**1. Contact-damage пил → модель лавы (САМОЕ ВАЖНОЕ, мой свежий код — главный регресс).**
- Убрать `start_contact_loop()`/`_contact_sweep()` (вечный `while`+`SLEEP`).
- floor_saw (live-тайл = свой `loc`): `AddElement(/datum/element/connect_loc, list(COMSIG_ATOM_ENTERED=…, COMSIG_ATOM_EXITED=…))` как в `pressure_plate.dm`. На Entered живого моба → `START_PROCESSING(SSobj, src)`; `process()` кусает всех живых на live-тайлах, при отсутствии живых → `PROCESS_KILL`.
- wall_saw (live-тайл = удалённый тайл перед стеной, меняется в цикле): зарегистрировать `connect_loc`/сигнал ENTERED на нужный турф ИЛИ процессить только пока `operating`/выехала. Idle-граза — только если на mouth-тайле кто-то есть (через Entered этого турфа), не поллингом.
- `process(seconds_per_tick)` использует `seconds_per_tick` для урона (как лава), а не фиксированный тик.

**2. Corrosion → SSprocessing + occupancy-gate + кэш турфов.**
- `/datum/corrosion_controller` → перевести на `SSprocessing` (`START_PROCESSING(SSprocessing, src)` / `process()` / `PROCESS_KILL`), убрать `_loop()`.
- **Gate на присутствие:** запускать обработку, только когда в `zone_area` есть живой моб; `PROCESS_KILL`, когда сектор пуст. Отслеживать вход/выход через `zone_area` Entered/Exited (areas шлют их) или счётчик оккупантов.
- **Кэш турфов:** список турфов сектора статичен после загрузки — кэшировать в `rust_sector.cached_turfs` (заполнять в `on_loaded()`), а `get_turfs()` отдаёт кэш. Сейчас `block()` строит 2116-список заново в `_sweep()`, `_register_mapped_hazards()`, `puzzles/puzzle.dm:75`, `set_turfs_to_area()`. Все перевести на кэш.
- Опционально: вместо скана contents всех турфов вести реестр корродируемых атомов (через Entered-хуки зоны). Сначала хватит gate+кэш.

**3. Hazard auto-loop (`start_loop`) → планировщик, а не прока-на-объект.**
- Либо `SSprocessing`-driven (объект сам процессится и решает, пора ли запустить цикл), либо централизованный тик в `sync_controller`, гоняющий свои hazards по расписанию (он и так группирует их по сектору). Дать `sync_controller` occupancy-gate.

**4. Load-time: `_register_mapped_hazards()` (`grid_manager.dm:204`)** сканит 2116 турфов на сектор при загрузке (25×2116 на старте). Разово, но добавляет round-start лаг. Итерировать `bounds`/контент шаблона или area.contents вместо полного `block()`. После п.2 уже будет на кэше.

### Чек-лист правил при рефакторе
- `START_PROCESSING(SSobj, src)` для objs (пилы/ловушки), `SSprocessing` для датумов (corrosion). `PROCESS_KILL` обязателен, когда работать не над чем.
- Никаких `set waitfor=FALSE` + вечный `while`+`SLEEP` для фоновой пер-объектной работы — это и есть текущий анти-паттерн.
- Player safety + mob-check перед уроном сохраняются.
- Контактный урон должен остаться (юзер просил idle-граза + cycle-страйк) — меняем ТОЛЬКО механизм доставки (Entered/process вместо поллинга), не поведение.
- После рефактора замерить: пустой лабиринт должен давать ~0 фоновой нагрузки (как пустая лава).
