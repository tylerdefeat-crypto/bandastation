...
ADMIN_VERB(give_banshee_kit_full, R_ADMIN, "Banshee KIT: Give", "Выдает персонажу полный набор способностей Банши.", ADMIN_CATEGORY_FUN, mob/living/target in world)
	if(QDELETED(target))
		return
...
ADMIN_VERB(banshee_unfreeze_radius_verb, R_ADMIN, "Banshee KIT: Un-Paralyze Radius", "Снимает стазис банши в радиусе.", ADMIN_CATEGORY_FUN)
...
ADMIN_VERB(remove_banshee_kit_full, R_ADMIN, "Banshee KIT: Remove", "Удаляет у персонажа все способности Банши и снимает паралич.", ADMIN_CATEGORY_FUN, mob/living/target in world)
	if(QDELETED(target))
		return

	// Список всех типов заклинаний для удаления
	var/list/banshee_spells = list(
		/datum/action/cooldown/spell/aoe/banshee_scream,
		/datum/action/cooldown/spell/aoe/banshee_death_wail,
		/datum/action/cooldown/spell/aoe/banshee_destructive_wail,
		/datum/action/cooldown/spell/pointed/banshee_focused_wail,
	...
	// Безопасно удаляем экшены через встроенный прок Remove()
	for(var/datum/action/cooldown/spell/S in target.actions)
		if(S.type in banshee_spells)
			S.Remove(target)
			removed_count++

	// Очищаем трейты оцепенения, если они были наложены этой системой
	if(isliving(target))
		var/mob/living/L = target
		L.remove_traits(list(TRAIT_INCAPACITATED, TRAIT_IMMOBILIZED, TRAIT_FLOORED), "banshee_scream")

	to_chat(user, span_notice("Успешно удалено [removed_count] способностей у [target.name]. Трейты оцепенения сброшены."))

	log_admin("[key_name(user)] удалил способности Банши у [key_name(target)].")
	message_admins("[key_name_admin(user)] удалил способности Банши у [key_name(target)].")
